...
class InternalDBManager:
    ...
    def list_db_connections(self) -> List[Dict[str, Any]]:
        with self._get_connection() as conn:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM database_connections ORDER BY name ASC")
            rows = cursor.fetchall()
            return [dict(row) for row in rows]

    def get_db_connection(self, conn_id: int) -> Optional[Dict[str, Any]]:
        with self._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
    ...
    def add_db_connection(self, connection_data: Dict[str, Any]) -> int:
        keys = ["name", "type", "host", "port", "database", "username", "password_encrypted", "ssl", "description", "is_active"]
        values = [connection_data.get(k) for k in keys]
        
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"INSERT INTO database_connections ({', '.join(keys)}) VALUES ({', '.join(['?'] * len(keys))})",
                values
            )
            conn.commit()
            return cursor.lastrowid

    def update_db_connection(self, conn_id: int, connection_data: Dict[str, Any]) -> bool:
        allowed_keys = ["name", "type", "host", "port", "database", "username", "password_encrypted", "ssl", "description", "is_active"]
        updates = []
        params = []
        ...
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(f"UPDATE database_connections SET {', '.join(updates)} WHERE id = ?", params)
            conn.commit()
            return cursor.rowcount > 0

    def delete_db_connection(self, conn_id: int) -> bool:
        with self._get_connection() as conn:
            cursor = conn.cursor()
