...
```sql
-- =============================================================================
-- PARAMETROS (EDITAR AQUÍ)
-- =============================================================================
DEFINE START_DATE = '31/03/2026 10:01h'
DEFINE END_DATE   = '07/04/2026 10:01h'
...
WITH params AS (
    SELECT
        TO_DATE('&START_DATE', 'DD/MM/YYYY HH24:MI"h"') AS start_date,
        TO_DATE('&END_DATE',   'DD/MM/YYYY HH24:MI"h"') AS end_date
    FROM dual
),
esquemes_valids AS (
...
esquemes_actius AS (
    ...
    JOIN esquemes_valids ev
      ON ev.owner = o.owner
    CROSS JOIN params p
    WHERE o.last_ddl_time >= p.start_date
      AND o.last_ddl_time <= p.end_date
)
SELECT
    t.owner                                            AS esquema,
    ...
    TO_CHAR(o.last_ddl_time, 'YYYY-MM-DD HH24:MI:SS')  AS data_modificacio_objecte
FROM all_tables t
JOIN dba_objects o
  ON o.owner = t.owner
...
WHERE t.owner IN (SELECT owner FROM esquemes_actius)
  ...
  AND o.last_ddl_time >= p.start_date
  AND o.last_ddl_time <= p.end_date
  AND NOT EXISTS (
      SELECT 1
...
esquemes_actius AS (
    ...
    JOIN esquemes_valids ev
      ON ev.owner = o.owner
    WHERE o.last_ddl_time >= SYSDATE - &DAYS_BACK
)
SELECT
    t.owner                                              AS esquema,
    t.table_name                                         AS taula,
...
WHERE t.owner IN (SELECT owner FROM esquemes_actius)
  AND t.temporary = 'N'
  AND t.iot_type IS NULL
  AND o.last_ddl_time >= SYSDATE - &DAYS_BACK
  AND NOT EXISTS (
      SELECT 1
      FROM all_indexes i
...
WHERE i1.owner IN (SELECT owner FROM esquemes_actius)
  ...
  AND (o1.last_ddl_time >= SYSDATE - &DAYS_BACK
       OR o2.last_ddl_time >= SYSDATE - &DAYS_BACK)
...
WHERE owner IN (SELECT owner FROM esquemes_actius)
  AND status = 'INVALID'
  AND last_ddl_time >= SYSDATE - &DAYS_BACK
